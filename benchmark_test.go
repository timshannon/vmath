...
func BenchmarkStructM3Mul(b *testing.B) {
	mat0 := new(vectormath.Matrix3)
	mat1 := new(vectormath.Matrix3)

	slice0 := []float32{23.41, 21.12, 0,
		214.23, 213.9821, -32.02,
		991.90, 75.123, -231.02}

	slice1 := []float32{23.41, 21.12, 0,
		214.23, 213.9821, -32.02,
		991.90, 75.123, -231.02}
	...
	for r := 0; r < 3; r++ {
		for c := 0; c < 3; c++ {
			//column major
			mat0.SetElem(c, r, slice0[(c*3)+r])
			mat1.SetElem(c, r, slice1[(c*3)+r])
		}
	}
	for i := 0; i < b.N; i++ {
		//SM3Mul(mat0, mat0, mat1)
		vectormath.M3Mul(mat0, mat0, mat1)
	}
